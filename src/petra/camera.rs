...
fn update_mouse(mut state: ResMut<State>, mut mouse_location: ResMut<MouseLoc>, cursor_move_events: Res<Events<CursorMoved>>){
...
fn update_camera(mouse_position: Res<MouseLoc>, mut query: Query<&mut Camera3dComponents>){
    for mut camera in &mut query.iter() {
        camera.rotation = Rotation::from_rotation_xyz(mouse_position.0.x(), mouse_position.0.y(), 0f32)
    }
}